...
// 1 ms interrupt
// Configure the timer clock frequency
// Calculation for 1 ms: OCR1A = (F_CPU / (Prescaler * frequency)) - 1
...
enum RequestCodes {
    ...
    SET_MONTH,           // Set month
    SET_YEAR,            // Set year
    SET_TEMPERATURE,     // Send temperature
...
enum PasswordResponse {
    ...
    PASSWORD_RESPONSE_CHANGE, // Password correct → change password
    PASSWORD_RESPONSE_UPLOAD  // Password correct → upload program
};

// Execute a Bluetooth request and update EEPROM and memory
void execRequest();

// Clear the serial receive buffer
void serialFlush();

// Disable the timeout system
void disableTimeout();

// Enable the timeout system
void enableTimeout();
