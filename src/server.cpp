#include "server.h"
#include "database.h"
#include "datatypes.h"
#include "display.h"
#include "global_vars.h"
#include "utils.h"
#include <Arduino.h>

#define TX_BUFFER_SIZE 200

// Read one byte from Serial into variable x, then decrement remaining size
#define SERIAL_READ_BYTE(x)                                                    \
  x = Serial.read();                                                           \
  size--;

// Same as SERIAL_READ_BYTE but checks buffer size before reading
#define SERIAL_READ_BYTE_S(x)                                                  \
  if (size == 0)                                                               \
    goto end;                                                                  \
  SERIAL_READ_BYTE(x)

// Read expected size "x" and validate remaining bytes in request
#define SERIAL_READ_SIZE(x)                                                    \
  SERIAL_READ_BYTE_S(x)                                                        \
  if (size < x)                                                                \
    goto bad;

// Read and discard one byte from Serial
#define SERIAL_READ()                                                          \
  Serial.read();                                                               \
  size--;

// Write a byte to the TX buffer and check for buffer overflow
#define SERIAL_WRITE_BYTE(x)                                                   \
  if (idx >= TX_BUFFER_SIZE) {                                                 \
    Serial.write(0x01);                                                        \
    Serial.write(BUFFER_OVERFLOW);                                             \
    Serial.flush();                                                            \
    goto end;                                                                  \
  }                                                                            \
  txBuffer[idx] = x;                                                           \
  idx++;

// Flush remaining unread bytes from request
#define FLUSH_REQUEST()                                                        \
  while (size != 0) {                                                          \
    Serial.read();                                                             \
    size--;                                                                    \
  }

boolean centuryFlag;
unsigned char txBuffer[TX_BUFFER_SIZE]; // Will store outgoing data
byte size;

void enableTimeout() { timeoutEnabled = true; }

void disableTimeout() {
  timeoutEnabled = false;
  timeoutCounter = TIMEOUT_COUNT;
}

bool ledOn = true;
void toggleLed() {
  // Small visual indicator for timeouts
  if (ledOn)
    digitalWrite(LED, HIGH);
  else
    digitalWrite(LED, LOW);
  ledOn = !ledOn;
}

void timeOut() {
  // Send timeout response
  Serial.write(0x01);
  Serial.write(TIMEOUT);
  Serial.flush();
  serialFlush();

  // Disable timer interrupt on OCR1A
  disableTimeout();
  toggleLed();
}

void serialFlush() {
  // Empty all unread serial bytes
  while (Serial.available()) {
    Serial.read();
  }
  size = 0;
}

void execRequest() {
  // Wait until at least one byte is available (size byte)
  if (Serial.available() < 1)
    return;

  // Read packet size if this is the beginning of a request
  if (size == 0) {
    size = Serial.read();
    enableTimeout();
  }

  // Wait for full request to arrive
  if (Serial.available() < size)
    return;

  disableTimeout();
  bool passwordSent = false;
  bool isPasswordCorrect = false;
  bool showSuccessMsg = false;

  // TX buffer index starts at 1 because index 0 will store packet length
  byte idx = 1;

  while (true) {
    SERIAL_READ_BYTE_S(tempByte);

    switch (tempByte) {

    case CONNECTED:
      lcd.home();
      lcd.clear();
      tone(BUZZER, BUZZER_FREQ, 200);
      lcd.print("    CONNECTED   ");
      delay(300);
      tone(BUZZER, BUZZER_FREQ, 200);
      delay(MSG_DELAY);

      // Refresh display depending on the active menu
      if (currentMenu == HOME) {
        initHome();
      } else {
        displayAlarm();
      }
      break;

    case GET_PROGRAM_TYPE:
      SERIAL_WRITE_BYTE(SET_PROGRAM_TYPE);
      SERIAL_WRITE_BYTE(eeprom.programType);
      break;

    case GET_ALARMS:
      SERIAL_WRITE_BYTE(SET_ALARMS);
      SERIAL_WRITE_BYTE(4 * eeprom.alarmCount); // Each alarm = 4 bytes

      for (byte i = 0; i < eeprom.alarmCount; i++) {
        SERIAL_WRITE_BYTE(eeprom.alarms[i].hour);
        SERIAL_WRITE_BYTE(eeprom.alarms[i].minute);
        SERIAL_WRITE_BYTE(eeprom.alarms[i].duration);
        SERIAL_WRITE_BYTE(eeprom.alarms[i].days);
      }
      break;

    case GET_DESCRIPTION:
      SERIAL_WRITE_BYTE(SET_DESCRIPTION);
      SERIAL_WRITE_BYTE(eeprom.descriptionLength);
      for (byte i = 0; i < eeprom.descriptionLength; i++) {
        SERIAL_WRITE_BYTE(eeprom.description[i]);
      }
      break;

    case GET_AUTHOR:
      SERIAL_WRITE_BYTE(SET_AUTHOR);
      SERIAL_WRITE_BYTE(eeprom.authorLength);
      for (byte i = 0; i < eeprom.authorLength; i++) {
        SERIAL_WRITE_BYTE(eeprom.author[i]);
      }
      break;

    case GET_HOUR:
      SERIAL_WRITE_BYTE(SET_HOUR);
      SERIAL_WRITE_BYTE(myRTC.getHour(use12hFormat, pmFlag));
      break;

    case GET_MINUTE:
      SERIAL_WRITE_BYTE(SET_MINUTE);
      SERIAL_WRITE_BYTE(myRTC.getMinute());
      break;

    case GET_SECOND:
      SERIAL_WRITE_BYTE(SET_SECOND);
      SERIAL_WRITE_BYTE(myRTC.getSecond());
      break;

    case GET_DAY_OF_WEEK:
      SERIAL_WRITE_BYTE(SET_DAY_OF_WEEK);
      SERIAL_WRITE_BYTE(myRTC.getDoW());
      break;

    case GET_DAY:
      SERIAL_WRITE_BYTE(SET_DAY);
      SERIAL_WRITE_BYTE(myRTC.getDate());
      break;

    case GET_MONTH:
      SERIAL_WRITE_BYTE(SET_MONTH);
      SERIAL_WRITE_BYTE(myRTC.getMonth(centuryFlag));
      break;

    case GET_YEAR:
      SERIAL_WRITE_BYTE(SET_YEAR);
      SERIAL_WRITE_BYTE(myRTC.getYear());
      break;

    case GET_TEMPERATURE:
      SERIAL_WRITE_BYTE(SET_TEMPERATURE);
      SERIAL_WRITE_BYTE((byte)roundf(myRTC.getTemperature()));
      break;

    case GET_STATE:
      SERIAL_WRITE_BYTE(SET_STATE);
      SERIAL_WRITE_BYTE(eeprom.state);
      break;

    case SET_PROGRAM_TYPE:
      SERIAL_READ_BYTE_S(tempByte);
      if (isPasswordCorrect) {
        eeprom.programType = tempByte;
        storeProgramType();
      }
      break;

    case SET_PASSWORD:
      // Password update requires full password in buffer
      if (size < PASSWORD_LEN)
        goto bad;

      if (isPasswordCorrect) {
        for (byte i = 0; i < PASSWORD_LEN; i++) {
          SERIAL_READ_BYTE(eeprom.password[i]);
        }
        storePassword();
      } else {
        // Discard incoming password bytes
        for (byte i = 0; i < PASSWORD_LEN; i++) {
          SERIAL_READ();
        }
      }
      break;

    case SET_ALARMS:
      // tempByte contains alarms byte-size
      SERIAL_READ_SIZE(tempByte);
      tempByte /= 4; // Convert bytes → number of alarms

      if (tempByte > MAX_ALARMS)
        goto bad;

      if (isPasswordCorrect) {
        eeprom.alarmCount = tempByte;

        for (byte i = 0; i < eeprom.alarmCount; i++) {

          SERIAL_READ_BYTE(tempByte);
          if (tempByte < 24) eeprom.alarms[i].hour = tempByte;
          else { getAlarms(); goto bad; }

          SERIAL_READ_BYTE(tempByte);
          if (tempByte < 60) eeprom.alarms[i].minute = tempByte;
          else { getAlarms(); goto bad; }

          SERIAL_READ_BYTE(tempByte);
          if (tempByte < 100) eeprom.alarms[i].duration = tempByte;
          else { getAlarms(); goto bad; }

          SERIAL_READ_BYTE(eeprom.alarms[i].days);
        }
        storeAlarms();
      } else {
        // Discard alarm bytes
        for (byte i = 0; i < tempByte; i++) {
          SERIAL_READ(); SERIAL_READ(); SERIAL_READ(); SERIAL_READ();
        }
      }
      break;

    case SET_DESCRIPTION:
      // tempByte contains description length
      SERIAL_READ_SIZE(tempByte);
      if (tempByte > MAX_DESCRIPTION_LEN)
        goto bad;

      if (isPasswordCorrect) {
        eeprom.descriptionLength = tempByte;
        for (byte i = 0; i < eeprom.descriptionLength; i++) {
          SERIAL_READ_BYTE(eeprom.description[i])
        }
        storeDescription();
      } else {
        // Discard description bytes
        for (byte i = 0; i < tempByte; i++) SERIAL_READ();
      }
      break;

    case SET_AUTHOR:
      SERIAL_READ_SIZE(tempByte);
      if (tempByte > MAX_AUTHOR_LEN)
        goto bad;

      if (isPasswordCorrect) {
        eeprom.authorLength = tempByte;
        for (byte i = 0; i < eeprom.authorLength; i++) {
          SERIAL_READ_BYTE(eeprom.author[i]);
        }
        storeAuthor();
      } else {
        for (byte i = 0; i < tempByte; i++) SERIAL_READ();
      }
      break;

    case SET_HOUR:
      SERIAL_READ_BYTE_S(tempByte);
      if (tempByte > 23)
        goto bad;
      if (isPasswordCorrect) myRTC.setHour(tempByte);
      break;

    case SET_MINUTE:
      SERIAL_READ_BYTE_S(tempByte);
      if (tempByte > 59)
        goto bad;
      if (isPasswordCorrect) myRTC.setMinute(tempByte);
      break;

    case SET_SECOND:
      SERIAL_READ_BYTE_S(tempByte);
      if (tempByte > 59)
        goto bad;
      if (isPasswordCorrect) myRTC.setSecond(tempByte);
      break;

    case SET_DAY_OF_WEEK:
      SERIAL_READ_BYTE_S(tempByte);
      if (tempByte < 1 || tempByte > 7)
        goto bad;
      if (isPasswordCorrect) {
        // 1=Sun, 2=Mon, ... 7=Sat
        myRTC.setDoW(tempByte);
      }
      break;

    case SET_DAY:
      SERIAL_READ_BYTE_S(tempByte);
      if (tempByte < 1 || tempByte > 31)
        goto bad;
      if (isPasswordCorrect) myRTC.setDate(tempByte);
      break;

    case SET_MONTH:
      SERIAL_READ_BYTE_S(tempByte);
      if (tempByte < 1 || tempByte > 12)
        goto bad;
      if (isPasswordCorrect) myRTC.setMonth(tempByte);
      break;

    case SET_YEAR:
      SERIAL_READ_BYTE_S(tempByte);
      if (tempByte > 99)
        goto bad;
      if (isPasswordCorrect) myRTC.setYear(tempByte);
      break;

    case SET_STATE:
      SERIAL_READ_BYTE_S(tempByte);
      if (isPasswordCorrect) {
        eeprom.state = tempByte;
        storeState();
      }
      break;

    // ------------------ Password Handling ------------------
    case POST_PASSWORD:
        showSuccessMsg=true;
    case POST_PASSWORD_UPLOAD:
    case POST_PASSWORD_CHANGE:
      passwordSent = true;
      if (size < PASSWORD_LEN) goto bad;
      isPasswordCorrect = true;
      for (byte i = 0; i < PASSWORD_LEN; i++) {
          byte b;
          SERIAL_READ_BYTE(b);
          if (eeprom.password[i] != b) isPasswordCorrect = false;
      }
      SERIAL_WRITE_BYTE(POST_PASSWORD_RESPONSE);
      if(tempByte == POST_PASSWORD){
          SERIAL_WRITE_BYTE(isPasswordCorrect ? PASSWORD_RESPONSE_CORRECT : PASSWORD_RESPONSE_INCORRECT);
      }else if(tempByte == POST_PASSWORD_UPLOAD){
          SERIAL_WRITE_BYTE(isPasswordCorrect ? PASSWORD_RESPONSE_UPLOAD : PASSWORD_RESPONSE_INCORRECT);
      }else if(tempByte == POST_PASSWORD_CHANGE){
          SERIAL_WRITE_BYTE(isPasswordCorrect ? PASSWORD_RESPONSE_CHANGE : PASSWORD_RESPONSE_INCORRECT);
      }
       
      break;

    case DISCONNECTED:
      lcd.home();
      lcd.clear();
      tone(BUZZER, BUZZER_FREQ, 200);
      lcd.print("  DISCONNECTED  ");
      delay(300);
      tone(BUZZER, BUZZER_FREQ, 200);
      delay(MSG_DELAY);

      if (currentMenu == HOME) {
        initHome();
      } else {
        displayAlarm();
      }
      break;

    default:
      goto bad;
    }
  }

bad:
  // Send error response
  Serial.write(0x01);
  Serial.write(BAD_REQUEST);
  Serial.flush();

end:
  // Send prepared TX buffer if any
  if (idx > 1) {
    txBuffer[0] = idx - 1;  // First byte = payload size
    Serial.write(txBuffer, idx);
    Serial.flush();
  }

  FLUSH_REQUEST();

  // If the request completed successfully
  if (showSuccessMsg && isPasswordCorrect) {
    lcd.home();
    lcd.clear();
    tone(BUZZER, BUZZER_FREQ, 200);
    lcd.print("      DONE      ");
    lcd.setCursor(0, 1);
    lcd.print("    SUCCESS !   ");
    delay(300);
    tone(BUZZER, BUZZER_FREQ, 200);
    delay(MSG_DELAY);

    if (currentMenu == HOME) initHome();
    else displayAlarm();
  }

  // If password was wrong
  if (passwordSent && !isPasswordCorrect) {
    lcd.home();
    lcd.clear();
    tone(BUZZER, BUZZER_FREQ, 200);
    lcd.print(" WRONG PASSWORD ");
    delay(300);
    tone(BUZZER, BUZZER_FREQ, 200);
    delay(MSG_DELAY);

    if (currentMenu == HOME) initHome();
    else displayAlarm();
  }
}
